...
def test_sync_command_parses() -> None:
    args = cli.parse_args(
        ["sync", "--session-id", "abc", "--project-name", "Override Project", "--test"]
    )
    assert args.command == "sync"
    assert args.session_id == "abc"
    assert args.project_name == "Override Project"
...
def test_send_payloads_returns_frontend_session_url_after_api_put() -> None:
    payload = {
        "share_id": "s1",
        "session": {"id": "s1", "project": "proj", "agent": "codex", "message_count": 1, "user_message_count": 1},
        "messages": [],
    ...
    assert result == {
        "sent_count": 1,
        "failed_count": 0,
        "sent": [{"share_id": "s1", "url": "https://history.caption.fyi/sessions/s1"}],
        "failures": [],
    }
...
def test_command_sync_project_name_overrides_payload_project(
    ...
    assert isinstance(result, list)
    assert len(result) == 2
    assert {payload["session"]["project"] for payload in result} == {"Matter Override"}


def test_command_sync_project_name_rejects_blank_value(tmp_path: Path) -> None:
    db_path = tmp_path / "sessions.db"
    make_agentsview_db(db_path)
    ...
    with pytest.raises(core.CliError, match="--project-name must not be blank"):
